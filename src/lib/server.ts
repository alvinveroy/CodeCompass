  ...
  try {
    ...
    server.tool(
      ...
      async (params: unknown) => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-base-to-string -- Custom stringification handles object case
    ...
    server.tool(
      ...
      async (params: unknown) => {
        ...
        if (typeof parsedParams === 'object' && parsedParams !== null && !('query' in parsedParams && typeof parsedParams.query === 'string')) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- Assigning to a property of a Record<string, unknown> after type check
